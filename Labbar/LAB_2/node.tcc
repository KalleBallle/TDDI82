...
template <typename T>
void Node<T>::insert(c_p<Node> node)
{
...
template <typename T>
void Node<T>::remove(c_p<Node> node)
{
...
template <typename T>
typename std::vector<c_p<Node<T>>>::const_iterator Node<T>::begin() const
{
    return neighbours.cbegin();
}

template <typename T>
typename std::vector<c_p<Node<T>>>::const_iterator Node<T>::end() const
{
...
std::vector<c_p<Node<T>>> get_all_nodes( c_p<Node<T>> root)
{
    ...
    std::queue<  c_p<Node<T>>> to_visit { };

    to_visit.push(root);
    while (!to_visit.empty())
    {
        c_p<Node<T>> node { to_visit.front() };
